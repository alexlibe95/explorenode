...
const realtimeOrders = (category) => {
  ...
  const sendUpdate = () => socket.send(JSON.stringify({cmd: 'update-category', payload: { category }}))
  if (socket.readyState === WebSocket.OPEN) {
    sendUpdate()
  } else if (socket.readyState === WebSocket.CONNECTING) {
    socket.addEventListener('open', sendUpdate, { once: true })
  }
}
